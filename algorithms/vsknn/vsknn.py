...
class VMContextKNN:
    ...
    def __init__(self, k, sample_size=1000, sampling='recent', similarity='cosine', weighting='div',
                 ...
                 time_key='Time', remove_outliers=False):
    ...
    def fit(self, data, items=None):
        ...
        if self.remove_outliers:
            dfagg = data.groupby([self.session_key]).agg(['count'])
            threshold = dfagg.quantile(0.995)[self.item_key]['count']
            mask = dfagg[self.item_key]['count'] < threshold
            dfagg = dfagg[mask]
            result = pd.DataFrame(dfagg[self.item_key]['count'].keys())
            data = data.set_index(self.session_key).join(result.set_index(self.session_key), how='inner')
    ...
    def predict_next(self, session_id, input_item_id, input_user_id=None, timestamp=0, skip=False,
                     type='view'):
        '''
...
#     full_path_to_csv_file = r'C:\Users\ZwartAlexde\Documents\UvA\thesis\thesis_sbr\data\prepared\vsknn\item_views_train_tr.0.txt'
...
#     training_df = df
#
#     test_path = r'C:\Users\ZwartAlexde\Documents\UvA\thesis\thesis_sbr\data\prepared\vsknn\item_views_train_valid.0.txt'
#     test_df = pd.read_csv(test_path, sep='\t',
#                           dtype={'SessionId': object, 'ItemId': 'str', 'Time': 'float64'})
#
#     # outputfilename = 'vsknn_py_predictions.txt'
#     latency_outputfilename = 'vsknn_py_latencies.txt'
#
#     params = VsknnParamSearch.get_parameters()
#
#     keys = params[0].keys()
#     with open('train_parameters.csv', 'w', newline='') as output_file:
#         dict_writer = csv.DictWriter(output_file, keys)
#         dict_writer.writeheader()
#         dict_writer.writerows(params)
#
#
#     param_out = open("parameters.csv", 'w')
#     dict_writer = csv.DictWriter(param_out, keys)
#
#
#     for ele, param in enumerate(params):
#
#         outputfilename = f'vsknn_py_predictions_{ele}.txt'
#
#         model = VMContextKNN(k=param['k'], sample_size=param['sample_size'], weighting=param['weighting'],
#                              weighting_score=param['weighting_score'], idf_weighting=param['idf_weighting'])
#         model.fit(training_df)
#         print(f'model.fit(training_df_{ele}) done')
